...
The problem: Session class stores messages in memory. When the process ends,
everything is lost. Users expect to resume conversations.

This builds on Session Management with key additions:
1. Persist session to disk as JSON
2. Load session from disk on startup
3. Resume conversations across process restarts
"""

import json
from concurrent.futures import ThreadPoolExecutor, as_completed
from agent_utils import FileSession, LLMOutput, Message, Session, Tool, ToolCall, ToolRegistry
...
RETURN ONLY THE JSON."""
    ...
    def _compact_session(self, session: Session):
        ...
        session.set_messages(
            [dict(role="user", content=compact_messages)]
        )
...
if __name__ == "__main__":
    ...
    agent = Agent(tools, max_prompt_tokens=5000)
